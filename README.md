...
echo " Acertou Miseravi!!! o n secreto é $numeroSecreto";
...
As vezesprecisamos interferir no laço enquanto ele está rodando 

- `break` => **Para Tudo!** Quebra laço interiro e avai embora 
- `continue` => **Pula a rodada** ele ignora o código daquela rodada especifica e pula logo para a próxima repetição.

Exemplo de aplicação do código: sistema de controle de elevador 
...
Exemplo de Aplicativo: Exibir todos os meses do ano 
...
$preços = [
   ...
   "caneta" => 5.50,
   "mochila" => 99.00
]; // vetor não ordenaado do tipo chave(key) => valor ===> coleção

// percorer o vetor usando o laço foreach
...
Se uma lógica foi escrita duas ou mais vezes dentro de um código, essa lógica devevirar uma função.
...
Uma função é como uma máquina: você coloca a matéria-prima (parãmetro), ela processa e devolve um produto final (retorno)
...
$textoNovo = str_replace("amaricana", "são paulo", $texto)
// "senai são paulo"
...
O tipo deve ser escrito antes do nome de cada parâmetro e o tipo da função deve ser escrito após os parenteses, precedido do ":". informando o que a função vai devolver.

Exemplo de uso de função de parâmetros tipados:
...
se uma função faz o trabalho interno e **Não retorna NADA**, dizemos que o retorno dela é "vazio" (`void`).
...
**Escopo** é o local do programa onde a variável pode ser armazenada/acessada. em PHP, uma variável criada fora de uma função pertence ao *escopo global*, uma variavel criada dentro de uma função pertende ao *escopo local*.
...
function sauder(string $nome):string{
   return "olá, $nome";
}
...
> Obs: Em arrays associativos, nos trocamos o nº do índice por Nomes (Chaves/Keys). A seta => significa "recebe"

**Arrays Multidimensionais (Banco de Dados na Memória)

é aqui que o "BackEnd" começa de verdade. o Array Multidimensional é o formato como os Bancos de Dados chegam como resposta as solicitações feitas pela API
...
echo $cliente[1]["email"]; //bruno@email.com

```

#### O Melho Amigo do Array: `O Foreach`
...
// vai implimir nome email de todos os Clientes do Array
...
São Usadas em Filtragem de Mapeamento de dados de um Array 
...
Antes de PHP processar qualquer informação, precisamos coletar informações no FrontEnd atráves de um `<form>`
...
3. name="nomeUsuario" -> A Etiqueta do dado: É o nome da chave que o PHP usará no array associativo ($POST["nomeUsuario"]).
...
-  **Método GET**: solicitar informações públicas e realizar buscas, mas altamente arriscadas para dados privados 
...
O método `GET` é utilizado quando a intanção do cliente é **buscar ou filtrar dados** sem alterar o estado do servidor. Os dados enviados via `GET` são anexados diretamente ao final da URL na forma de uma **Query String**
